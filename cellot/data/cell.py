...
class AnnDataDataset(Dataset):
    def __init__(self, adata, obs=None, categories=None, include_index=False,
                 dim_red=None):
        self.adata = adata
...
def read_tupro_cohort_labels(config):
    table = pd.read_csv(config.data.paths.table, index_col=0)
    samples = sorted([line.strip() for line in open(config.data.paths.cohort)])
    table = table.reindex(samples)

    mask = select_cohort(table, **dict(config.data.selection.select))
    cohort = label_cohort_as_train_or_eval(
        table.index[mask],
        holdout=config.data.selection.get('holdout'),
        trainset=config.data.selection.get('trainset'),
        evalset=config.data.selection.get('evalset'),
    )
    return cohort.reindex(samples).fillna('removed')


def read_tupro_cohort(config, trainonly=True):
    cohort = read_tupro_cohort_labels(config)
    root = Path(config.data.paths.root)
    layer = config.data.paths.layer
    groups = cohort.groupby(cohort).groups
    groups = {
            k: [root/sample/f'{layer}.h5ad' for sample in v]
            for k, v
            in groups.items()
        }

    assert 'train' in groups.keys()
    assert {'train', 'eval', 'removed'}.issuperset(groups.keys())

    data = read_merged_anndata(config, pathlist=groups['train'])

    if not trainonly:
        evaldata = read_merged_anndata(config, pathlist=groups['eval'])
        evaldata.obs['split'] = 'ood'
        data = data.concatenate(evaldata)
